...
if __name__ == "__main__":
    ...
    p_ft_end = 0.3
    p_ft_steps = 15
    p_ft_list = np.linspace(p_ft_start, p_ft_end, p_ft_steps).tolist()
    p_ft = p_ft_list[run_index] if run_index < len(p_ft_list) else p_ft_list[-1]
    ...
    config = {
        ...
        "L": 15,
        "T" : 30,
        "d_mlp_list": [100], # [int(round(d_mlp))]
        "alpha": alpha,
        "rho": 1.0,
        "beta": 1.0,
    ...
    config["N_alpha"] = int(config["alpha"] * config["D"]**2)
    config["N_ft"] = int(config["N_alpha"] * config["p_ft"])
    config["N_train"] = config["N_alpha"] - config["N_ft"]
