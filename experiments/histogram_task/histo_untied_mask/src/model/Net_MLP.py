...
class Net(torch.nn.Module):
    ...
    def forward(self, x, delta_in=0.0):
        ...
        if self.mask_diag:
            mask = torch.eye(self.T, device=self.device).bool() # Création du masque diagonal
            A = A.masked_fill(mask, -float('inf'))  # Remplissage avec -infini pour annuler la probabilité
