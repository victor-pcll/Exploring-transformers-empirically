...
def fine_tune_student(config, train_dataset, student_trained):
    ...
    for p in student_trained.parameters(): p.requires_grad = False
    student_trained.W_Q.weight.requires_grad = True
    student_trained.W_K.weight.requires_grad = True

    optimizer = torch.optim.Adam(
        [student_trained.W_Q.weight, student_trained.W_K.weight],
        lr=config["learning_rate_fine_tune"]
    )
    ...
    for t in range(config["max_fine_tune_iter"]):
        ...
        for X_batch, y_batch in train_loader:
            ...
            if config["lambda"] > 0:
                reg_loss = config["lambda"] * (
                    torch.sum(student_trained.W_Q.weight ** 2) +
                    torch.sum(student_trained.W_K.weight ** 2)
                )
            else:
                reg_loss = 0.0
    ...
    with torch.no_grad():
        ...
        if config["lambda"] > 0:
            reg_loss_final = config["lambda"] * (
                torch.sum(student_trained.W_Q.weight ** 2).item() +
                torch.sum(student_trained.W_K.weight ** 2).item()
            )
        else:
            reg_loss_final = 0.0
