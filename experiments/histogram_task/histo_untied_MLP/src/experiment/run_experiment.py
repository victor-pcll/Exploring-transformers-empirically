...
def run_experiment(config):
    ...
    for mlp_dim in config["d_mlp_list"]:
        ...
        for _ in range(config["samples"]):
            ...
            WQ = convert_to_numpy(student_trained.W_Q.weight)
            WK = convert_to_numpy(student_trained.W_K.weight)
            rank_before = 0.5 * (
                np.linalg.matrix_rank(WQ) +
                np.linalg.matrix_rank(WK)
            )
            ...
            WQ = convert_to_numpy(student_fine_tuned.W_Q.weight)
            WK = convert_to_numpy(student_fine_tuned.W_K.weight)
            rank_after = 0.5 * (
                np.linalg.matrix_rank(WQ) +
                np.linalg.matrix_rank(WK)
            )
            rank_after_runs.append(rank_after)
            weights_runs.append({"WQ": WQ, "WK": WK})
