...
class Net(torch.nn.Module):
    ...
    def forward(self, x, delta_in=0.0):
        ...
        if self.mask_diag:
            # Création du masque diagonal (True sur la diagonale)
            mask = torch.eye(self.T, device=self.device).bool() # shape (T, T) broadcasté automatiquement sur (N, T, T)
            A = A.masked_fill(mask, -float('inf')) # Le softmax transformera -inf en 0
