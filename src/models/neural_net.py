...
class Net(torch.nn.Module):
    ...
    def __init__(
        ...
        self.fc1 = torch.nn.Linear(
            input_dim, hidden_dim, bias=False, device=self.device, dtype=self.dtype
        )
    ...
    def forward(self, x, delta_in=0.0):
        ...
        x = attention_matrix - trace_part * torch.eye(
            self.L, device=self.device, dtype=self.dtype
        )

        # Inject input noise if delta_in > 0
        if delta_in > 0.0:
            M = torch.full(
                (self.L, self.L), 1.0 / np.sqrt(2), device=self.device, dtype=self.dtype
            )
            M.diagonal().fill_(1)
            eps = torch.normal(0.0, 1.0, x.shape, device=self.device, dtype=self.dtype)
            i, j = torch.triu_indices(
                row=self.L, col=self.L, offset=1, device=self.device
            )
            eps[..., j, i] = eps[..., i, j]
